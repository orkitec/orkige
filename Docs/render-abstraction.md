...
| `planarReflection` | yes | no | opt-in MIRROR reflection of the actual scene (sky + terrain + objects) in the water surface, rather than just the sky IBL cubemap it already samples. classic = the textbook mirror pass: a dedicated camera reflected across the surface plane (normal +Y) renders the scene into a reflection RenderTexture (the water surface hidden, geometry below the plane custom-near-clipped) that the water shader samples at the fragment's ripple-perturbed screen UV, blended over the base look by `reflectionStrength` - basic render-to-texture, so it reaches the GLES2/WebGL mobile/web path (runtime-gated where a context cannot render to a texture); it composes with screen-space refraction. next = NOT available (the native HlmsPbs planar-reflection component is not compiled into this build), so the reflection stays the sky IBL cubemap and a requested planar reflection logs one honest line |
