...
| `planarReflection` | yes | yes | opt-in MIRROR reflection of the actual scene (sky + terrain + objects) in the water surface, rather than just the sky IBL cubemap it already samples. Both flavors render the textbook mirror pass: a camera reflected across the surface plane (normal +Y at `planeHeightY`) renders the scene into a reflection RenderTexture with the water surface hidden, which the water material samples; `reflectionStrength` drives how mirror-like the surface reads. classic = a hand-authored GLSL program samples the mirror at the fragment's ripple-perturbed screen UV - basic render-to-texture, so it reaches the GLES2/WebGL mobile/web path (runtime-gated where a context cannot render to a texture); a Vulkan/GLES/WebGL context answers false pending its shader variant. next = the native Ogre-Next PlanarReflections subsystem: a reflected mirror camera renders the scene (below the water render queue) into an RTT the HlmsPbs water datablock samples in its reflection slot (higher strength raises F0 + darkens the water albedo so the mirrored scene dominates); driven per frame from the window workspace update, capability-present on the desktop-capable Metal/Vulkan targets. Both compose with screen-space refraction; off/unsupported keeps the byte-stable sky-reflection look |
...
- **classic (SHIPS, desktop GL3Plus):** a grab-pass. A window-sized
  `TU_RENDERTARGET` re-renders the scene from the main camera each frame with the
  refractive water HIDDEN (a `RenderTargetListener` over the entity registry),
  and the water surface renders through a hand-authored GLSL program that samples
  that grab at the normal-perturbed screen UV. Basic render-to-texture only, so
  it reaches the GLES2/WebGL1 mobile/web path in principle — but the program is
  authored for desktop GLSL, so a Vulkan/GLES/WebGL context runtime-gates the
  capability false (byte-stable) pending its shader variant + an on-device proof
