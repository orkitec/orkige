...
| gui HUD (widgets + UiAtlas/UiRenderer on DrawLayer2D) | yes | yes (one draw batch per screen, selfchecked) |
...
| editor (ImGui on DrawLayer2D) | yes | **yes** |
| pixel-level colour parity with classic (WYSIWYG) | — (the reference) | yes (`render_backend_parity`; gamma-space passthrough) |
| jumper sample (C++ gui HUD) | yes | no (classic boot block only; the HUD itself is flavor-neutral) |
