...
| `RenderWorld.h` | `RenderWorld` | root node, node/content factories, ambient light, dynamic-shadow control (the `RenderCaps::DynamicShadows` capability + the `setShadowQuality` off/low/medium/high knob over `core_util/ShadowPreset.h`), sky/fog atmosphere (the `RenderCaps::SkyDome` capability + `setAtmosphere(AtmosphereDesc)` — sun-linked atmospheric sky dome + object fog on next; classic evaluates the SAME shared sky model per-pixel on its dome (GLSL core/ES3 targets, vertex-gradient fallback on GLES2/Vulkan) and runs the SAME atmospheric object-fog formulas on generated materials + the water programs (`AtmosphereFogSrs`), with a curve-matched fixed-function scene fog as the non-generated fallback), `queryRay` AABB picking (`RayQueryHit`: distance/node/userPointer), `createVertexColourCubeMesh` (the backend cube-mesh service - the editor's "Create Cube" resource every scene-loading app needs; classic impl reuses PrimitiveUtil) |
...
| `VectorMesh.h` | `VectorMesh` | vector-shape content (VectorShape/VectorAnimationComponent): a world-space vertex-coloured indexed triangle list refilled from a CPU array (`setMesh`, the SpriteBatch contract) or a SECTION list (`setMeshSections` — one draw per section; flat sections share the "VectorFill" unlit alpha datablock, TEXTURED cutout sections carry UVs and bind the per-texture SPRITE material/datablock, reused wholesale), zOrder on the SAME painter window as SpriteQuad/SpriteBatch. classic = `ManualObject` OT_TRIANGLE_LIST, one Ogre section per facade section; next = v2 `ManualObject` SCENE_DYNAMIC likewise. Triangles + per-texture runs come from the renderer-free `core_util/VectorTessellator` |
| `LineMesh.h` | `LineMesh` | 3D LINE content (LineComponent + the immediate-mode `draw` debug table): a world/local-space vertex-coloured connected polyline (`TOPOLOGY_STRIP`) or independent-segment list (`TOPOLOGY_SEGMENTS`) refilled from a CPU array (`setLines`) with a same-count dynamic fast path (`updateVertices` — `ManualObject::beginUpdate`, the VectorMesh precedent, one-tick-deferred first update on next). Depth-tested against the scene or an on-top overlay (`setDepthTest`); casts no shadow, never picked; hairline only (no width knob — GL line width is dead, a quad-expansion thickness pass is future). classic = `ManualObject` OT_LINE_STRIP/OT_LINE_LIST + the shared unlit "VertexColour"/"VertexColourOverlay" material (the cube/grid look reused); next = v2 `ManualObject` SCENE_DYNAMIC + the matching HlmsUnlit datablock in the default 3D item queue. The MODERN revival of the deleted 2010 DynamicLines utility (built on the facade, not resurrected). Debug primitives ride the pure `engine_graphic/DebugDrawBuffer` (TTL/capacity + primitive→segment expansion) flushed into ONE dynamic LineMesh per frame by the engine-owned `engine_graphic/DebugDraw` (ScreenFade/ScreenShake precedent — player-ticked, editor-inert, zero cost until first draw) |
...
**Not** in the facade, by design: an authored material *system* — node graphs or OGRE
material scripts (per CLAUDE.md, keep materials simple/generated). What the facade DOES
carry is a *generated*-material surface: sprite material generation inside `SpriteQuad`,
unlit fixup on `MeshInstance`, and the `createMaterial` (PBS `RenderMaterialDesc`, usually
from a `.omat`) + `createWaterMaterial` (`RenderWaterDesc`) services — each a handful of
generated HlmsPbs/Unlit datablocks on next / RTSS-generated materials (Cook-Torrance
metal-rough) on classic. Also out of the facade: overlays/imgui (editor-private glue),
frame *events* (stay on the core event system around `renderOneFrame`), log forwarding
(engine service, not renderer), Gorilla/gui.
...
| hemisphere ambient (`RenderWorld::setAmbientHemisphere`) | yes (`RenderCaps::HemisphereAmbient` = true) — a generated-shader hemisphere-ambient sub-render-state (`HemisphereAmbientSrs`) evaluates the SAME two-colour sky/ground term PER PIXEL against the surface normal on the generated lit materials (`mix(lower, upper, dot(hemisphereDir, N) * 0.5 + 0.5)`, the reflectance recovered to display space to match next's gamma-space ambient consumption — tolerance parity, not the native path). REGISTERED subset only at the edges: the fixed-function fallback (no shader generator) + imported-mesh materials still see one flat AVERAGED ambient, since classic OGRE's `SceneManager::setAmbientLight` takes a single colour | yes (native two-colour sky/ground term, `setAmbientLight(upper, lower, dir)`) |
...
and the surface samples it at a normal-perturbed screen UV, so what sits under
the water bends/wobbles. Authored through `RenderWaterDesc::screenSpaceRefraction`
+ `refractionStrength` and the reflected `WaterComponent` props (Inspector /
serialization / Lua / MCP), capability-gated: OFF or unsupported → the byte-stable
Stage-1 look + one honest log line. NOT depth-graded deep→shallow transmission
(still a later stage — that needs a depth-graded pass).

- **classic (SHIPS, desktop GL3Plus + WebGL2/GLES3):** a grab-pass. A
  window-sized `TU_RENDERTARGET` re-renders the scene from the main camera each
  frame with the refractive water HIDDEN (a `RenderTargetListener` over the entity
  registry), and the water surface renders through a hand-authored GLSL program
  that samples that grab at the normal-perturbed screen UV. The program ships two
  variants (`waterGlslProfile`): desktop GL core (`#version 150`) and GLSL ES 3.0
  (`#version 300 es`), selected by the RTSS target language, so the path reaches
  the **WebGL2/GLES3** web+device floor as well — gated on the `glsl300es` probe
  exactly like the IBL stage. The **GLES2/WebGL1** floor keeps rendering the
  byte-stable Stage-1 look (the grab program indexes the sky cubemap's mip chain,
  which needs ES3 sampling); a Vulkan/Metal context still answers false pending
  its own shader variant + an on-device proof run. Verified by
  `water_refraction_looks_right` (a wavy-displacement vs straight-baseline pixel
  probe) and, on web, the `export_web_water` boot check —
  `RenderSystemClassic.cpp`.
