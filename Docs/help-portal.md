...
- `/help/changelog` — the full release history: every commit, grouped by the
  day it landed, each day headed by the ordered version identity a build of it
  carries. It is the only portal page with no committed source file: the
  repository's git history is the record, and the page renders from it at
  deploy time through the composition in `Util/orkige_nightly_package.py` —
  ...
  for. Both jobs that build the site check out with `fetch-depth: 0`; from a
  shallow clone the page says the history it lists is truncated, and with no
  git history at all it says nothing is listed rather than rendering empty.
