...
returned. `screenshot_game` inlines the same way once the running game's frame
is back — normally inside its own reply; if the player needs longer the reply
says `pending:"1"` and the frame is read off the confirmed path instead.
...
The `toolSpecs` table in `EditorControlServer.cpp` is the advertised tool list —
the source of truth, mirrored below. Each tool maps onto an existing
`EditorCore` method or an `EditorDocument` free function; nothing bypasses the
verb handler. (`tools/call` will run any verb the handler knows, so the
advertised set is the discoverable subset, not a hard limit.)

| Tool | Maps to |
|------|---------|
| `get_state` | project/scene/dirty/selection/play-mode snapshot (+ the prefab-edit stage: `edit_context` (`scene`/`prefab`), and while a prefab is staged `prefab_path`/`prefab_root`/`prefab_dirty` plus `scene_path`/`scene_dirty` re-reported from the STASHED scene the close restores; + `build_status`/`build_target`/`build_errors` for compile-on-Play; while a play session runs, the streamed-music snapshot: parallel `music_ids`/`music_files` arrays plus a `music_info` string per track — `"playing positionSec durationSec baseGain groupVolume effectiveGain loop"`, streamed on `MSG_STATS`, empty when nothing plays; plus the runtime perf summary: `frame_ms`, the engine-level allocation counters `alloc_per_frame`/`alloc_peak` with the per-subsystem breakdown in parallel `alloc_tags`/`alloc_counts`, and `profile_seq` — see `get_profile`; plus the running game's STORE: `store_provider` (the installed provider's name, empty when the game installed none — the ordinary answer on a platform with no store), `store_ready`, `store_pending`, `store_ad_free` and `store_owned` (the LOGICAL product ids the player owns — [monetization.md](monetization.md)); plus its ADS: `ads_provider` (empty when the game installed none — the ordinary answer, since the engine mediates and ships no ad network), `ads_ready`, `ads_consent` (`not_gathered`/`granted`/`denied` — the ordering constraint the surface turns on), `ads_test_mode`, `ads_takeover` (`1` while a fullscreen advert covers the app, during which the world is deliberately NOT advancing) and `ads_banner_height` in window pixels; unhappy ad paths are pinned with `set_cvar` on `ads.sim` while the simulated surface is installed; plus `transaction_open` — `1` while a `begin_transaction` atomic-edit bracket is open) |
...
| `screenshot_game(path)` | screenshot the RUNNING game's frame (`MSG_SCREENSHOT`) to a path on the EDITOR's disk — desktop play AND **Play in Browser** (a page's capture rides the debug link back as chunked image data, `MSG_SCREENSHOT_DATA`, and the editor writes it). The PNG normally comes back in the reply as an inline image block with `screenshot_ok`; a slower answer replies `pending:"1"` + `prev_screenshot_seq` → poll `get_state`. Refused for a simulator/phone session (their captures stay on the device) |
...
`.oshape` in process — no interpreter, no subprocess). Create and place with `create_object` +
...
from pose SVGs with `tools/shapecook --targets`. Observe the deform on the
...
editor's **Preview** tab (both share the one `GamePreviewStage`, so an edit
...
- `screenshot_game(path)` captures the RUNNING game's next rendered frame (the
  most-cited verification primitive). `path` is on the EDITOR's disk. Two roads
  reach it, and the RUNTIME picks: a desktop player shares this filesystem and
  writes the file itself; a browser page cannot, so it sends the PNG's bytes over
  the debug link in a numbered chunk sequence and the editor writes them at
  `path`. Either way the verb waits a bounded moment for the answer, so the usual
  reply is `{ accepted:"1", path, screenshot_ok:"1", screenshot_seq }` with the
  frame inlined as an image content block. If the answer is still in flight the
  reply carries `pending:"1"` + `prev_screenshot_seq` and the historical async
  contract stands: poll `get_state` until `screenshot_seq` exceeds it, then
  `screenshot_ok`/`screenshot_path` carry the result. A damaged chunk sequence
  FAILS CLOSED — a `[remote] screenshot FAILED` line naming the damage, never a
  truncated PNG. A simulator or phone session is refused: its capture stays on
  the device.
...
tools/call screenshot_game { "path":"/tmp/frame.png" }  // → { screenshot_ok:"1" } + the frame inlined
...
- `"browser"` is an export-serve-open that becomes a LIVE play session once
  ...
  `record_trace` (the page's in-memory filesystem never reaches the
  editor's disk, and a trace is a file) and `reload_script`/`reload_ui`/
  ...
  `screenshot_game` DOES work:
  the page's capture cannot reach this disk either, so the image itself
  travels over the link in chunks and the editor writes it — see the live
  parity cockpit walkthrough in `Docs/mcp-workflows.md`. Gated (`target_states:"gated"`) until the
...
beside it. The desktop platforms and `ios-simulator` only; an `android` or `web`
payload lives inside an archive the runner cannot walk, and is refused by name
rather than reporting a pass over zero tests.
