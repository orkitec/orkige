...
bool TiXmlDocument::LoadFile( const char* filename, TiXmlEncoding encoding, IphoneDir iphoneDir )
{
...
#ifdef ORKIGE_NDS
bool TiXmlDocument::LoadFile( FSFile* file, TiXmlEncoding encoding )
#else
...
#endif
{
	...
	FS_SeekFile(file, 0, FS_SEEK_END);
...
//	memory[0] = 0;
	...
	SDK_ASSERT(file);
	SDK_ASSERT(memory);
    if (FS_ReadFile(file, memory, length) != length) {
	#else
	...
	const char* lastPos = memory;
	const char* p = memory;

	memory[length] = 0;
	while( *p ) {
		assert( p < (memory+length) );
		if ( *p == 0xa ) {
...
bool TiXmlDocument::LoadFromMemory( char * buffer, long length )
{
    // Delete the existing data:
    Clear();
    location.Clear();
