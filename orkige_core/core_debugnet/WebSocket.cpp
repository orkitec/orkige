...
namespace Orkige
{
	namespace
	{
		...
		//---------------------------------------------------------
		//! @brief SHA-1 over one in-memory message (FIPS 180-1). The
		//! handshake needs exactly one digest of a ~60-byte string, so a
		//! compact self-contained implementation beats a crypto dependency;
		//! this is NOT a general-purpose cryptographic facility.
		void sha1(unsigned char const * data, std::size_t length,
	...
	namespace WebSocketUtil
	{
		...
		String computeAcceptKey(String const & secWebSocketKey)
		{
			...
			sha1(reinterpret_cast<unsigned char const *>(salted.data()),
				salted.size(), digest);
