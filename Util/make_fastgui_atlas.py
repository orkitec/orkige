#!/usr/bin/env python3
"""Generate the FastGUI texture atlas (PNG + .ogui) - stdlib only.
...
Usage: make_fastgui_atlas.py <output_dir> [atlas_name]
       (default atlas name: fastgui_default)
"""
...
def build_atlas(out_dir, atlas_name):
    ...
    ogui.append("# generated by Util/make_fastgui_atlas.py - do not edit")
    ogui.append("[Texture]")
...
def fail(msg):
    print(f"make_fastgui_atlas: VALIDATION FAILED - {msg}", file=sys.stderr)
    sys.exit(1)
...
def main():
    if len(sys.argv) < 2:
        ...
        sys.exit(2)
    out_dir = sys.argv[1]
    atlas_name = sys.argv[2] if len(sys.argv) > 2 else "fastgui_default"
    os.makedirs(out_dir, exist_ok=True)
    png_path, ogui_path, canvas, occupied = build_atlas(out_dir, atlas_name)
    validate(png_path, ogui_path, canvas, occupied)
    print(f"make_fastgui_atlas: wrote {ogui_path} + {png_path} (validated)")
