...
namespace Orkige
{
	...
	//! @brief reserve + rasterise the terminal agent-badge glyphs into the DEFAULT
	//! UI-font atlas: one PRIVATE-USE codepoint per recognised agent, its pixels
	//! GENERATED by EditorTerminalSession's pure badge maker (a signature-tinted
	//! mark). Call AFTER the fonts are loaded and BEFORE the atlas texture is
	//! uploaded (it forces the atlas build to pack + fill the custom rects). A
	//! no-op when no base font exists. @p uiFontPixels is the base UI font's pixel
	//! size (points x content scale) so the badge cell matches the icon glyph
	//! metrics and tabs line up.
