...
namespace SceneOptimizer
{
	...
	bool Application::init(Orkige::String const & commandLine)
	{
		...
		FileUtils::SetCurrentPath (FileUtils::DialogBrowseFolder ("Choose resource directory").c_str());
		...
		this->gsm->registerState(onew(new MenuState("Menu")));
		...
		if( !this->engine->setup("Scene Optimizer", Engine::SHOW_IFERROR, this->externalWindowHandle, this->topLevelWindowHandle))
			return false;

		this->engine->createDefaultCameraAndViewport();

		this->fastGuiManager = onew(new FastGuiManager(onew(new FastGuiFactory())));
