...
DotSceneLoader::DotSceneLoader() : mSceneMgr(0) 
{
	...
	this->enableCustomLod = false;
	this->useLodBoundingBoxAsDefault = false;
	this->customLodDistance = 100;
}
...
void DotSceneLoader::parseDotScene(const Ogre::String &SceneName, const Ogre::String &groupName, Ogre::SceneManager *yourSceneMgr, Ogre::SceneNode *pAttachNode, const Ogre::String &sPrependNode, Ogre::String _namePrefix)
{
	...
	std::string scene = stream->getAsString();
...
void DotSceneLoader::processUserData(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{

	Ogre::String partentName = this->namePrefix;
	partentName.append(pParent->getName());
	
	rapidxml::xml_node<>* pElement = XMLNode->first_node("property");
	while(pElement)
	{
		Ogre::String name = this->namePrefix;
		name.append(getAttrib(pElement, "name", "notFound"));
		
		// nodeProperty(const Ogre::String &node, const Ogre::String &propertyName, const Ogre::String &value, const Ogre::String &type)
		this->nodeProperties.push_back(
			nodeProperty(
				//getAttrib(XMLNode->parent(), "name"),
				pParent->getName(),
				getAttrib(pElement, "name", "notFound"),
				getAttrib(pElement, "data", "notFound"),
				getAttrib(pElement, "type", "notFound")
			)
		);
...
void DotSceneLoader::processLight(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
    // Process attributes
    Ogre::String name = this->namePrefix;
	name.append(getAttrib(XMLNode, "name"));
    ...
    pLight->setVisible(true);//getAttribBool(XMLNode, "visible", true));
    pLight->setCastShadows(getAttribBool(XMLNode, "castShadows", true));

    rapidxml::xml_node<>* pElement;

//	don't process position twice (pe)
//    // Process position (?)
//    pElement = XMLNode->first_node("position");
//    if(pElement)
...
void DotSceneLoader::processCamera(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
    ...
    this->cameras.push_back(pCamera);
...
void DotSceneLoader::processNode(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
    ...
    {
		Ogre::String nodeName = this->namePrefix;
		nodeName.append(name);
		
        // Provide the name
        if(pParent)
            pNode = pParent->createChildSceneNode(nodeName);
        else
            pNode = mAttachNode->createChildSceneNode(nodeName);
    }
	...
	if(pElement)
	{
		...
		//pElement = pElement->next_sibling("userData");
	}
...
void DotSceneLoader::processEntity(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
    ...
    try
    {
		boost::algorithm::to_lower(meshFile);	//FIXME: this is here until all levels are fixed (pe)

		// don't do this anymore (pe)
		//if(!Ogre::ResourceGroupManager::getSingleton().resourceExistsInAnyGroup(meshFile))
		//{
		//	if(boost::ends_with(meshFile, ".mesh"))
		//	{
		//		boost::replace_all(meshFile, ".mesh", ".MESH")´;
		//	}
		//}

		if(this->enableCustomLod)
		{
			Orkige::MeshUtil::setupCustomLodMesh(meshFile, this->customLodDistance, this->useLodBoundingBoxAsDefault, "_lod", true, m_sGroupName);
		}

        pEntity = mSceneMgr->createEntity(name, meshFile);
		pEntity->setQueryFlags(PPR::PICK_DEFAULT);		// otherwise it contains 0xFFFFFFFF (pe)
    ...
    {
		Ogre::LogManager::getSingleton().logMessage("[DotSceneLoader] Error loading an entity! Reason: " + e.getDescription());
    }
...
void DotSceneLoader::processSubEntity(rapidxml::xml_node<>* XMLNode, Ogre::Entity *pParent)
{
	// Process attributes
	Ogre::Real index = getAttribReal(XMLNode, "index", -1.f);
	if(index == -1.f)
		return;

	Ogre::String materialName = getAttrib(XMLNode, "materialName");
	...
	try
	{
		subEntity = pParent->getSubEntity((unsigned int)index);
	}
	catch(Ogre::Exception & e)
	{
		Ogre::LogManager::getSingleton().logMessage("[DotSceneLoader] Error loading an entity! Reason: " + e.getDescription());
...
void DotSceneLoader::processParticleSystem(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
    ...
    {
        Ogre::LogManager::getSingleton().logMessage("[DotSceneLoader] Error creating a particle system! Reason: " + e.getDescription());
    }
}
...
void DotSceneLoader::processFog(rapidxml::xml_node<>* XMLNode)
{
	...
	// this is mixed up in the exporter
    if(sMode == "none")
        mode = Ogre::FOG_NONE;
    else if(sMode == "exp")
        mode = Ogre::FOG_EXP2;
