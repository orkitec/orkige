...
{
	// a padded buffer must land in the file as its packed pixels alone -
	// decoded with the reference inflater and compared byte for byte
	const int w = 2, h = 3;
	...
	const std::vector<unsigned char> stream = collectIdat(png);
	REQUIRE_FALSE(stream.empty());
	const std::size_t rawSize = static_cast<std::size_t>(h) * (1 + w * 4);
	std::vector<unsigned char> raw(rawSize);
	uLongf rawLen = static_cast<uLongf>(rawSize);
	REQUIRE(uncompress(raw.data(), &rawLen, stream.data(),
		static_cast<uLong>(stream.size())) == Z_OK);
	...
	for (int y = 0; y < h; ++y)
	{
		...
		decoded.insert(decoded.end(),
			raw.begin() + static_cast<long>(rowAt + 1),
