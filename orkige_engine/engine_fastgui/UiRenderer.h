...
//! @file UiRenderer.h
//! @brief the fastgui 2D renderer: a retained scene of rects, captions
...
namespace Orkige
{
	...
	//! FastGuiManager (one per FastGuiView/atlas). update() is the once-
	...
	public:
		//! @param atlas outlives the screen (FastGuiManager owns both)
		...
		//! compositing order among ALL 2D layers (FastGuiManager's
		...
		//! per-frame poke (FastGuiManager's FrameStarted): follow window
