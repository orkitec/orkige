      ...
      // Push our style into D3D.
    {
        ...
        || m_mode == BITMAP_CLAMP)
      {
        ...
        else
        {
          ...
          m_pd3dDevice->SetTexture(0, m_d3d_textures[static_cast<const bitmap_info_d3d*>(m_bitmap_info)->m_texture_id]);
          ...
          float inv_width = 1.0f / static_cast<const bitmap_info_d3d*>(m_bitmap_info)->m_original_width;
          float inv_height = 1.0f / static_cast<const bitmap_info_d3d*>(m_bitmap_info)->m_original_height;
...
/ *
        m_pd3dDevice->Clear(1, &rect,D3DCLEAR_TARGET,
          D3DCOLOR_RGBA(background_color.m_r, background_color.m_g, background_color.m_b, background_color.m_a),
          0.0f, 0 );* /
  ...
  void  fill_style_color(int fill_side, gameswf::rgba const & color)
    ...
    // Intended for textured glyph rendering.
  {
    ...
    // Set texture.
    m_pd3dDevice->SetTexture(0, m_d3d_textures[static_cast<const bitmap_info_d3d*>(bi)->m_texture_id]);
    m_pd3dDevice->SetTextureStageState( 0, D3DTSS_COLOROP,   D3DTOP_SELECTARG2);
    m_pd3dDevice->SetTextureStageState( 0, D3DTSS_ALPHAOP,   D3DTOP_SELECTARG1);
  ...
  bool is_visible(const gameswf::rect& bound){return true;}

  void   open(){}

  void   draw_triangle_list(const void* coords, int vertex_count){}

  gameswf::video_handler* create_video_handler(void){return NULL;}

  bool test_stencil_buffer(const gameswf::rect &,Uint8){return true;}
...
#include <Ogre.h>
#include <engine_graphic/GraphicManager.h>
gameswf::render_handler*  create_render_handler_ogre_d3d()
// Factory.
{
	Ogre::RenderWindow* renderWindowBase = Orkge::GraphicManager::getSingleton().getRenderWindow().get();/ *Ogre::Root::getSingleton().getAutoCreatedWindow();* /
	assert(renderWindowBase);//whoops :)
	Ogre::D3D9RenderWindow* renderWindow = static_cast<Ogre::D3D9RenderWindow*>(renderWindowBase);
	assert(renderWindow);
	IDirect3DDevice* device = renderWindow->getD3DDevice();
	gameswf::render_handler* hndlr = gameswf::create_render_handler_d3d( device );
	return hndlr;
}

void updateOgreRenderer()
{
	Ogre::RenderWindow* renderWindowBase = Orkge::GraphicManager::getSingleton().getRenderWindow().get();
/ *	renderWindowBase->swapBuffers(true);* /
}
